...
/**
 * THE HOUSE AFTER HOURS — the dark editorial homepage, as a route rather than
 * a replacement.
 *
 * WHY /after-hours AND NOT /. The live homepage sells. This is a direction
 * that has been through three iterations in a week and each one arrived with
 * factual errors in the product data, so it goes up beside the real homepage
 * and gets looked at on the real domain with the real catalogue before it
 * takes the front door. Promoting it is a one-line change: move this file to
 * src/app/page.tsx and move the current page.tsx here.
 *
...
export default function AfterHoursPage() {
  return (
    <div className="bg-emerald-deep text-cream">
      ...
      <section id="room-collection" className="section bg-cream text-charcoal">
        <div className="shell">
          <Reveal className="grid gap-6 md:grid-cols-[1.4fr_1fr] md:items-end md:gap-16">
            <div>
              <p className="room-label room-label-dk">Room 03 · The Collection</p>
              <h2 className="headline-house mt-5 text-balance text-charcoal">
                Private tools. Public power.
              </h2>
            </div>
            <p className="max-w-[34ch] text-[0.9375rem] leading-relaxed text-charcoal/70">
          ...
          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {IN_THE_MAKING.map((item, i) => (
              <Reveal key={item.name} as="article" delay={(i % 3) * 90} className="m-0">
                <div className="relative flex aspect-[2/3] flex-col justify-end overflow-hidden border border-charcoal/12 bg-shell p-6">
                  <span
                    aria-hidden
                    className="absolute inset-0 flex items-center justify-center font-serif text-4xl font-light text-charcoal/12"
                  >
                    {item.name}
                  </span>
                  <span className="relative">
                    <span className="room-label room-label-dk block">
                      {item.n} · {item.archetype}
                    </span>
                    <span className="mt-2 block font-serif text-[1.75rem] font-light leading-none text-charcoal">
                      {item.name}
                    </span>
                    <span className="mt-2 block text-[0.8125rem] text-charcoal/70">
                      {item.descriptor}
                    </span>
                    <span className="mt-3 flex items-center gap-2 text-[0.5rem] uppercase tracking-[0.16em] text-charcoal/70">
                      <span aria-hidden className="block h-[0.3rem] w-[0.3rem] rounded-full bg-charcoal/25" />
                      In the making
                    </span>
                    <span className="mt-4 block border-t border-charcoal/12 pt-3">
                      <Link href="#room-invitation" className="hairline text-charcoal">
                        Join the waitlist
                      </Link>
                    </span>
                  </span>
          ...
          <Reveal className="mt-16 border-t border-charcoal/12 pt-10">
            <p className="room-label room-label-dk">On sale now · The LALALOCA Collection</p>
              ...
              {products.map((product) => (
                <li key={product.slug}>
                  <Link href={`/products/${product.slug}`} className="group block text-center">
                    ...
                    <span className="room-label room-label-dk mt-5 block">{product.archetype}</span>
                    <span className="mt-2 block font-serif text-2xl leading-none text-charcoal">
                      {product.name}
                    </span>
                    <span className="mt-2 block font-serif text-lg leading-snug text-charcoal/75">
                      {product.hero}
                    </span>
                    <span className="mt-3 block text-sm text-charcoal">
