  ...
  return (
    <>
      ...
      <section className="section bg-charcoal text-shell">
        <div className="shell">
          <h2 className="subhead max-w-[20ch]">How we handle what you send us.</h2>
          <ul className="mt-10 grid gap-px bg-shell/15 sm:grid-cols-2 lg:grid-cols-4">
            {STORY_STANDARD.map((item, index) => (
              <li key={item.title} className="bg-charcoal p-6">
                <span className="eyebrow text-bronze">
                  {String(index + 1).padStart(2, "0")}
                </span>
                <h3 className="mt-4 font-serif text-2xl leading-tight">{item.title}</h3>
                <p className="mt-3 text-sm leading-relaxed text-shell/80">{item.detail}</p>
              </li>
            ))}
          </ul>
        </div>
